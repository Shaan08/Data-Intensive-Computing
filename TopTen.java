...
public class TopTen {
	//parsing xml inout data to string
    ...
    public static class TopTenMapper extends Mapper<Object, Text, NullWritable, Text> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        	//use the function for parsed values
            additionToRepToRecordMap(value, repToRecordMap);
    ...
    private static void additionToRepToRecordMap(Text value, TreeMap<Integer, Text> repToRecordMap) {
        Map<String, String> parsed = transformXmlToMap(value.toString());
        String userId = parsed.get("AccountId");
        String reputation = parsed.get("Reputation");

        // Add this record to our map with the reputation as the key
        // also check is the row has null data using valid function
        if (valid(userId) && valid(reputation)) {
            repToRecordMap.put(Integer.parseInt(reputation), new Text(value)); 
        }
    }
    //check if any parsed string row has null values.
